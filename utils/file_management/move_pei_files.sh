...
# Define paths
INPUT_FOLDER="/Users/claudiacastrillonalvarez/Desktop/github/EHRatioAnalysis/PEI_data/PEI_images"
OUTPUT_FOLDER="/Users/claudiacastrillonalvarez/Desktop/IMAGES_FIJI/PEI"
JSON_FILE="pei_image_dict.json"
    ...
    if [[ "$annotation" -eq 1 ]]; then
        ...
        for patient_folder in "$INPUT_FOLDER"/*/; do
            if [[ -f "$patient_folder$image_name" ]]; then
                ...
                # Create patient-specific folder in output
                patient_output_folder="$OUTPUT_FOLDER/$patient_name PEI TIFF"
